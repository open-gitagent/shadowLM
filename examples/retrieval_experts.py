...
# A Llama in the local mlx loop (ungated, 4-bit). To use the gated original
# meta-llama/Llama-3.2-1B-Instruct, set an HF token first: slm.hub.set_token(...)
# or `export HF_TOKEN=...`.
...
run = model.finetune(
    ...
    save_steps=50,            # checkpoint a version every 50 steps (50/100/150)
...
# 4. checkpointing: save_steps wrote a version every 50 steps -----------------
print("=== saved versions (from save_steps=50) ===")
for c in run.checkpoints():
    print(f"  {c.label:>8}  {c.path}")

# 5. the adapter dir carries its index — reload it anywhere -------------------
fresh = slm.load(MODEL, adapter=run.checkpoint)
print("\n=== reloaded from the adapter (index travels with it) ===")
